...
// SECURED: Added the 'user' prop
export default function OperatorDashboard({ user }) {
  // === GRANULAR PERMISSION CHECKS ===
  const isAdmin = user?.role === 'ADMIN';
  const canInitStk = isAdmin || user?.permissions?.includes('INITIATE_STK_PUSH');
  const canPayout = isAdmin || user?.permissions?.includes('CREATE_PAYOUT');
  const canRefund = isAdmin || user?.permissions?.includes('PROCESS_REFUNDS');
  
  // If they have at least one of these permissions, they can see the left column form
  const canUseManualTrigger = canInitStk || canPayout || canRefund;
  ...
  // ==========================================
  // 2. TRANSACTION TRIGGER STATE
  // ==========================================
  const [processingMode, setProcessingMode] = useState('single');
  // Default to STK Push if allowed, otherwise fallback to the next available option
  const [transactionType, setTransactionType] = useState(canInitStk ? 'STK Push' : (canPayout ? 'B2C Salary' : 'B2C Refund'));
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 bg-slate-50 min-h-full flex flex-col max-w-[1400px] mx-auto w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
        ...
        {/* LEFT COLUMN: TRIGGER FORM - SECURED */}
        {canUseManualTrigger && (
              ...
              <div className="flex justify-center gap-6">
                ...
                <button 
                  onClick={() => { 
                    ...
                    setStatus('idle'); 
                    setMessage(''); 
                    // Automatically switch to Salary if they switch to batch mode, assuming they have permission
                    setTransactionType(canPayout ? 'B2C Salary' : 'B2C Refund'); 
                  }}
              ...
              <form onSubmit={handleExecute} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      {/* SECURED DROPDOWN: Options render only if the user has the required permission */}
                      {processingMode === 'single' && canInitStk && <option value="STK Push">C2B STK Push (Collection)</option>}
                      {canPayout && <option value="B2C Salary">B2C Salary (Disbursement)</option>}
                      {canRefund && <option value="B2C Refund">B2C Refund (Disbursement)</option>}
                    </select>
        ...
        {/* SECURED DYNAMIC WIDTH: Expands to fill the screen if the trigger form is hidden */}
        <div className={`${canUseManualTrigger ? 'lg:col-span-2' : 'lg:col-span-3'} bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col transition-all duration-300`}>
